...
class ValueNode:
    def __init__(self, token: lexer.Token, value_type: str, value: str):
        ...
        self.type = value_type # NOTE: This is the type generated by the lexer (e.g "INTEGER", "FLOAT")
                               #       and not the final type of the value (e.g "u8", "f32")
        self.value = value

    def __str__(self):
        return f"ValueNode({self.type}, {self.value})"
...
class ParameterNode:
    ...
    def __str__(self):
        return f"ParameterNode({self.name}, {self.type})"
...
class VarNode:
    def __init__(self, name_token, var_type_token, name: str, var_type: str, value: ExprNode):
        self.name_token = name_token # Identifier token
        self.var_type_token = var_type_token # Identifier token (type) - NOTE: this is actally final type (e.g "u8", "f32")
        self.name = name
        self.type = var_type
        self.value = value

    def __str__(self):
        return f"VarNode({self.name}, {self.type}, {self.value})"
...
class ContextManager:
    def __init__(self, file):
        self.file = file
        self.stack = []
        self.globals = {} # name: VariableNode or FuncNode
        self.require_defined_in_future_dict = {} # name: VariableNode or FuncNode
        self.level = 0
    ...
    def is_running_conditionally(self):
        for i in range(len(self.stack) - 1, -1, -1):
            if self.stack[i][0] in ["if", "loop"]:
                return True
    ...
    def parse_expr():
        ...
        def parse_unary():
            ...
            # check if the expression starts with a unary operator
                ...
                for t in ["STRING", "CHAR", "NULL"]:
                    if value.type == t:
                        raise ParserError(operator, f"Cannot apply unary operator '{operator.value}' to {t.lower()}")
